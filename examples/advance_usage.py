...
def setup_logging(level=logging.INFO):
   ...
   logging.basicConfig(
      ...
      handlers=[
         logging.FileHandler('nepse_client.log'),
...
class NepsePortfolioAnalyzer:
   ...
   def __init__(self, use_async: bool = False):
      ...
      self.client = AsyncNepse() if use_async else Nepse()
...
class NepsePriceAlertSystem:
   ...
   def add_alert(
      ...
      logger.info(
         f"Alert added: {symbol} "
         f"{'above' if above else 'below'} {target_price}"
      )
   ...
   def check_alerts(self):
      ...
      for alert in self.alerts:
         ...
         try:
               ...
               condition_met = (
                  (alert['above'] and current_price >= alert['target_price']) or
...
async def monitor_market_continuously(interval_seconds: int = 60):
   ...
   while True:
      try:
         ...
         logger.info(
               f"Market: {status.get('isOpen')} | "
               f"Turnover: {summary.get('totalTurnover')}"
         )
